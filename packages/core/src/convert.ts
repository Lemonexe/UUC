...
function Convert() {
	...
	*/
	...
	this.convert = function (input, target) {
		...
		input = this.beautify(input);
		target = this.beautify(target);
		...
		if (isTargetCurly) {
			return this.processTargetCurly(iObj, tObj);
		}
		...
		if (isNaN(num)) {
			throw this.msgDB['ERR_NaN_result'];
		}
		...
		if (corr !== true) {
			dim += '*' + this.vector2text(corr, true);
		}
	...
	this.fullConversion = function (input, target) {
		...
		try {
			...
			this.status = 2;
			this.messages = [err];
		} // downgrade status to 2 and only the error message will be shown
	...
	this.rationalizeField = function (obj, curly) {
		// optional curly = if curly brackets are legal in their normal sense
		const that = this;
		return crawl(obj);
	...
	this.reduceField = function (obj) {
		...
		function crawl(arr) {
			...
			if (!(arr[0] instanceof Q) && !Array.isArray(arr[0])) {
				throw that.msgDB['ERR_operator_misplaced'](arr[0]);
			}
			let i, res;
			...
			arr = subcrawl(arr, ['*', '/'], (res, sign, q) =>
				sign === '*' ? that.multiply(res, q) : that.divide(res, q)
			);
			...
			for (i = 0; i < arr.length; i += 2) {
				if (arr.hasOwnProperty(i + 1)) {
					if (arr[i + 1] === '+' && arr[i + 2] instanceof Q) {
						res = that.add(res, arr[i + 2]);
					} else if (arr[i + 1] === '-' && arr[i + 2] instanceof Q) {
						res = that.subtract(res, arr[i + 2]);
					} else {
		...
		function subcrawl(arr, signs, callback) {
			let i;
			let res = null;
			let arr2 = []; // current result, new reduced array of Q
			for (i = 0; i < arr.length; i += 2) {
				if (!arr.hasOwnProperty(i + 1)) {
					break;
				}
	...
	this.processCurly = function (arr, curly) {
		...
		if (!curly || units.length !== 1 || units[0].power !== 1 || arrs.length > 1) {
			throw Err113;
		}
		['+', '-', '/', '^'].forEach((op) => {
			if (arr.includes(op)) {
		...
		if (arrs.length > 0) {
			...
			if (!checkZeros(res.v)) {
				throw this.msgDB['ERR_cbrackets_dim_mismatch'](units[0].unit.id);
			}
			nums.push(res.n);
		}
		if (nums.length > 1) {
		...
		if (i === -1) {
			throw this.msgDB['ERR_unknown_unitfun'](units[0].unit.id);
		}
	...
	this.processTargetCurly = function (iObj, tObj) {
		...
		if (x !== 0) {
			throw this.msgDB['ERR_cbrackets_illegal'];
		} // no number in {target}
	...
	*/
	...
	this.power = function (q1, q2) {
		...
		if (!checkZeros(q2.v)) {
			throw this.msgDB['ERR_power_dim'];
		}
	...
	this.multiply = (q1, q2) =>
		new Q(
			q1.n * q2.n,
			q1.v.map((o, i) => o + q2.v[i])
		);
	this.divide = (q1, q2) => this.multiply(q1, this.power(q2, new Q(-1)));

	this.add = function (q1, q2) {
		// check dimension
		if (!checkZeros(q1.v.map((o, i) => o - q2.v[i]))) {
			throw this.msgDB['ERR_dim_mismatch'];
		}
	...
	*/
	...
	this.checkTargetNumbers = function (obj) {
		...
		for (let o of obj) {
			if (typeof o === 'number' && o !== 1) {
				this.warn(this.msgDB['WARN_targetNumber']);
				return;
			}
		}
	};
	...
	this.format = function (output, params) {
		...
		if (output.dim.search(/^\d/) > -1) {
			output.dim = ' * ' + output.dim;
		}

		let dp = params.digits,
			df = params.fixed,
			num = output.num;
		...
		} else if (params.spec === 'fixed') {
			output.num2 = num.toFixed(df || 0);
